...
export default function CheckoutPage() {
  ...
  if (!hasHydrated) {
    return (
      <div className="min-h-screen bg-[#F8FAF3] flex items-center justify-center">
        <div className="flex flex-col items-center gap-2">
          <Loader2 className="animate-spin text-[#3D4A24]" />
          <p className="text-sm text-[#4F6130]/60 font-medium">
            Cargando pedido...
          </p>
        </div>
      </div>
  ...
  if (estado === "exito") {
    ...
    return (
      <main className="min-h-screen bg-[#F8FAF3] flex items-center justify-center p-6">
        <div className="flex flex-col items-center gap-6 text-center max-w-sm">
          ...
          <div>
            <h1 className="text-2xl font-semibold text-[#3D4A24] tracking-tight">
              ¡Pedido confirmado!
            </h1>
            <p className="mt-2 text-sm text-[#4F6130]/70 leading-relaxed">
              Tu pedido está en camino. Lo recibirás en:{" "}
              <span className="font-semibold text-[#3D4A24]">
                {form.direccion}
              </span>
            </p>
          </div>

          <div className="w-full rounded-2xl border border-[#C4D4A0]/40 bg-white p-4 text-left space-y-2">
            <p className="text-xs font-semibold uppercase tracking-wider text-[#4F6130]/50">
              Resumen del pedido
            </p>
            <div className="flex justify-between text-sm text-[#3D4A24]">
          ...
          <div className="flex w-full gap-3">
            <Button
              variant="outline"
              className="flex-1 rounded-xl border-[#C4D4A0]/40 text-[#3D4A24] hover:bg-[#EEF3E6]"
              onClick={() => router.push("/")}
            >
              Volver al menú
            </Button>
            ...
            <Button
              className="flex-1 gap-2 rounded-xl bg-[#3D4A24] text-[#F4F7EE] hover:bg-[#4F6130]"
  ...
  return (
    <main className="min-h-screen bg-[#F8FAF3]">
      {/* ── Top bar ── */}
      <header className="sticky top-0 z-10 border-b border-[#C4D4A0]/30 bg-[#F8FAF3]/90 backdrop-blur-sm">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
          <button
            onClick={() => router.back()}
            className="flex items-center gap-1.5 text-sm text-[#4F6130]/70 transition-colors hover:text-[#3D4A24]"
          >
            <ArrowLeft size={15} strokeWidth={2} />
            Volver
          </button>

          <div className="flex items-center gap-2 text-[#3D4A24]">
          ...
          <Badge
            variant="outline"
            className="border-[#C4D4A0]/40 bg-[#EEF3E6] text-[10px] text-[#4F6130]"
          >
      ...
      <div className="mx-auto grid max-w-6xl gap-8 px-6 py-10 lg:grid-cols-[1fr_400px]">
        ...
        <div className="space-y-6">
          ...
          >
            <div className="mb-4 flex items-start gap-3 rounded-xl border border-[#C4D4A0]/40 bg-[#EEF3E6]/60 px-4 py-3">
              <MapPin size={14} className="text-[#4F6130] mt-0.5 shrink-0" />
              <p className="text-xs text-[#4F6130]/80 leading-relaxed">
                Este pedido es exclusivo para{" "}
                <span className="font-semibold text-[#3D4A24]">delivery</span>.
            ...
            <div className="mt-5">
              ...
              {form.metodoPago === "tarjeta" && (
                <div className="space-y-4">
                  ...
                  <div className="flex items-center gap-2 rounded-xl border border-[#C4D4A0]/40 bg-[#EEF3E6]/40 px-3 py-2.5">
                    <Lock size={13} className="text-[#4F6130] shrink-0" />
        ...
        <div className="space-y-4">
          <div className="sticky top-24">
            <Card className="overflow-hidden border-[#C4D4A0]/40 bg-white shadow-none">
              <CardHeader className="border-b border-[#C4D4A0]/40 bg-[#EEF3E6]/40 px-5 py-4">
                <CardTitle className="flex items-center gap-2 text-base text-[#3D4A24]">
              ...
              <CardContent className="p-0">
                {items.length === 0 ? (
                  <p className="px-5 py-8 text-center text-sm text-[#4F6130]/60">
                    No hay productos en el carrito.
                  </p>
                ) : (
                  <ul className="divide-y divide-[#C4D4A0]/30">
                    {items.map((item) => (
                      <li key={item.id} className="flex gap-3 px-5 py-4">
                        <div className="relative h-14 w-14 shrink-0 overflow-hidden rounded-xl border border-[#C4D4A0]/40">
                          <Image
                            src={item.imagen}
                            alt={item.nombre}
                            fill
                        ...
                        <div className="flex flex-1 flex-col gap-1.5">
                          <div className="flex items-start justify-between gap-2">
                            <p className="text-[13px] font-semibold leading-tight text-[#3D4A24]">
                              {item.nombre}
                            </p>
                            <button
                              aria-label={`Eliminar ${item.nombre}`}
                              onClick={() => eliminarProducto(item.id)}
                              className="shrink-0 rounded p-0.5 text-[#4F6130]/30 transition-colors hover:text-red-400"
                            >
                          ...
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-1 rounded-full border border-[#C4D4A0]/40 bg-[#F8FAF3] px-1.5 py-0.5">
                              <button
                                aria-label="Disminuir cantidad"
                                onClick={() =>
                                  actualizarCantidad(item.id, item.cantidad - 1)
                                }
                                className="flex h-5 w-5 items-center justify-center rounded-full text-[#4F6130] hover:bg-[#EEF3E6]"
                              >
                                <Minus size={10} strokeWidth={2.5} />
                              </button>
                              <span className="w-5 text-center text-xs font-semibold tabular-nums text-[#3D4A24]">
                                {item.cantidad}
                              </span>
                              <button
                                ...
                                className="flex h-5 w-5 items-center justify-center rounded-full text-[#4F6130] hover:bg-[#EEF3E6] disabled:opacity-30"
                              >
                ...
                {items.length > 0 && (
                  <div className="border-t border-[#C4D4A0]/40 bg-[#EEF3E6]/30 px-5 py-4 space-y-2">
                    <div className="flex justify-between text-sm text-[#4F6130]/70">
                      <span>Subtotal</span>
                    ...
                    <div className="flex items-baseline justify-between">
                      <span className="text-sm font-semibold text-[#3D4A24]">
                        Total
                      </span>
                      <span className="text-xl font-bold tracking-tight text-[#3D4A24]">
                        S/ {total.toFixed(2)}
                      </span>
                    </div>
                  </div>
            ...
            <Button
              ...
              className="
                ...
                bg-[#3D4A24] py-6 text-base font-semibold tracking-wide
                text-[#F4F7EE] shadow-none transition-all
                hover:bg-[#4F6130] active:scale-[0.98]
            ...
            <p className="mt-3 text-center text-[11px] text-[#4F6130]/50">
...
function SectionCard({
  ...
  return (
    <Card className="border-[#C4D4A0]/40 bg-white shadow-none">
      <CardHeader className="border-b border-[#C4D4A0]/30 px-6 py-4">
        <CardTitle className="flex items-center gap-2 text-[15px] font-semibold text-[#3D4A24]">
          <span className="text-[#4F6130]">{icon}</span>
          {title}
        </CardTitle>
      </CardHeader>
...
function FieldGroup({
  ...
  return (
    <div className="space-y-1.5">
      <Label
        htmlFor={id}
        className="flex items-center gap-1.5 text-xs font-medium text-[#4F6130]"
      >
        {icon && <span className="text-[#4F6130]">{icon}</span>}
        {label}
      </Label>
  ...
  return (
    <Label
      ...
      className={`
        ...
        ${
          selected
            ? "border-[#4F6130] bg-[#EEF3E6] text-[#3D4A24]"
            : "border-[#C4D4A0]/40 bg-white text-[#4F6130] hover:border-[#4F6130]/40 hover:bg-[#F8FAF3]"
        }
      `}
    >
      <RadioGroupItem id={`pago-${value}`} value={value} className="sr-only" />
      <span className={selected ? "text-[#4F6130]" : "text-[#4F6130]/50"}>
        {icon}
      </span>
...
function inputCls(hasError: boolean) {
  return `
    border-[#C4D4A0]/40 bg-[#F8FAF3] text-[#3D4A24] placeholder:text-[#4F6130]/30
    focus-visible:ring-1 focus-visible:ring-[#4F6130] focus-visible:border-[#4F6130]
